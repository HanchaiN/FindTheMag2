...
from libs.pyboinc.rpc_client import RPCClient, init_rpc_client
    ...
    while current_retries < max_retries:
        ...
        try:
            ...
            if not str(parsed):
                print_and_log(
                    "ERROR",
                    "Warning: Error w RPC command %s: %s" % (full_command, parsed),
                )
                continue
        except Exception as e:
            log.error("Error w RPC command %s %s", full_command, e)
            continue
        else:
            return parsed
    return None
    ...
    for task in task_list:
        try:
            ...
            if "active_task" not in task or started:
                print_and_log(
                    "DEBUG", "Cancelling unstarted task %s", task, quiet=quiet
                )
...
async def prefs_check(
    ...
    if (max_gb < min_gb and max_gb != 0) or used_max_gb < (min_gb - expected_gb_used):
        if not testing:
            print_and_log(
                "ERROR",
                "BOINC is configured to use less than %fGB, this tool will not run with <%fGB allocated in order to prevent requesting base project files from projects too often."
                % (min_gb, min_gb),
            )
            print_and_log(
                "ERROR",
                'If you have configured BOINC to be able to use >=%fGB and still get this message, it is because you are low on disk space and BOINC is responding to settings such as "don\'t use greater than X% of space" or "leave x% free"'
                % min_gb,
            )
            if not scripted_run:
